...
import { triggerExplosion, RememberSpotMap } from './GameItems/explosion.js';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const loaded = await preloadAssets(assets);
        ...
        // --- NEW CODE: THE WALKIE-TALKIE LISTENER ---
    RPC.register("updateTile", (data) => {
        console.log(`Walkie-Talkie: Someone changed tile (${data.x}, ${data.y}) to ${data.type}`);
        
        // 1. Update the local visuals for everyone else
        gridApi.setType(data.x, data.y, data.type);
        
        // 2. If this computer is the Host, permanently save the map 
        // so any players who join the game late don't see the ghost powerup!
        if (isHost()) {
        ...
        for (const id in remotePlayers) {
            ...
            if (remoteBomb) {
                ...
                if (remoteBomb.id !== lastBombIds[id]) {
                    ...
                    // Remember the original tile type where the bomb is placed
                    const originalType = gridApi.getType(remoteBomb.x, remoteBomb.y);
                    RememberSpotMap.set(`${remoteBomb.x},${remoteBomb.y}`, originalType);
                    
                    gridApi.setType(remoteBomb.x, remoteBomb.y, TILE_TYPE.bomb);
                    console.log(`Remote bomb placed at (${remoteBomb.x}, ${remoteBomb.y}) by player ${id}. Original tile type was: ${originalType}`);
